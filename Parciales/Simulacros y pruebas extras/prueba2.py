...
def ind_nesima_aparicion(s: list[int], n: int, elem: int) -> int:
    ...
    for i in range(len(s)):
        if s[i] == elem:
            ...
            if n == contador:
                return i
    if apariciones(s, elem) < n:
        return -1
def apariciones(s: list[int], elem: int) -> int:
    res: int = 0
    for i in s:
        if i == elem:
            res += 1
...
def posicion(caballo:str, lista:list) -> int:
    ...
    for e in lista:
        if e == caballo:
            indice += 1
    return indice
def posiciones(caballo: str, carreras: dict) -> list:
    lista_carreras: list = []
...
def matriz_capicua(m: list[list[int]]) -> bool:
    longitud: int = len(m[0])
    res: bool = True
    for i in range(len(m)):
        for j in range(len(m[0])):
            if m[i][j] != m[i][longitud -1 - j]:
                res = False
                break
    return res
